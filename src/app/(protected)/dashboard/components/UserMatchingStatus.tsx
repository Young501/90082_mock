    ...
    >
      ...
      <VStack align="stretch" gap={4}>
        ...
        <HStack gap={4} mb={{base: "20px", lg: "40px"}}>
          ...
          <Box w={{base: "90px"}} h={{base: "90px"}} borderRadius="50%" border={`10px solid ${getBorderColor()}`} display="flex" alignItems="center" justifyContent="center">
              ...
              {participant.messages.map((message, index) => (
                <Box key={message.id || index}>
                  <HStack gap={6} alignItems="start" w="100%">
                    <Box w={{base: "8px", md: "12px", lg: "23px"}} mt={2} h={{base: "8px", md: "12px", lg: "23px"}}>
          ...
          : userType === 'partner' ? (
            <VStack align="stretch" gap={2}>
              <Text fontSize={{base: "18px", lg: "22px"}} fontWeight="700">
                ...
                {Array.isArray(participant.match_info?.matched_with) && participant.match_info?.matched_with?.map((matchedWith: any, index: number) => (
                <Box key={matchedWith.id || index} border="1px solid #2CA9DF" px={6} py={2} borderRadius="10px" w="100%" display="flex" alignItems="center" justifyContent="start" gap={6}>
                  <Box w={{base: "40px"}} h={{base: "40px"}} borderRadius="50%" border={`5px solid #DC2626`} display="flex" alignItems="center" justifyContent="center">
                  <Avatar.Root
                    width="35px"
                    height="35px"
                    borderRadius="50%"
...

              {
                participant.match_info?.is_matched && userType === 'student' && (
                  ...
                  <Box display="flex" alignItems="center" gap={2}>
                    <Image src="/assets/matched.svg" alt="check-circle" width={34} height={34} />
                    <Text fontSize={{base: "16px", lg: "20px"}} color="#000000" fontWeight="600">
                    Student is MATCHED to  {getMatchedWithName()}
                    </Text>
                  </Box>

                  <Text fontSize={{base: "16px", lg: "20px"}} color="#000000" fontWeight="400">
                    {formatDate(participant.messages?.[participant.messages.length - 1]?.sent_at || "")}
                  </Text>
                  </VStack>
  ...
  userType === 'student' && (
              <Box>
                  ...
                  <HStack align="stretch" gap={3} w="100%" justifyContent="space-between">
                    ...
                    Do you want to unmatch this {userType === 'student' ? 'student' : 'organisation'}?
                    </Text>
                    <Button
                      bg="#D9D9D9"
                      color="#000000"
