...
export const EmailCustomizationPreview: React.FC<EmailCustomizationPreviewProps> = ({
  opportunityId,
  userType,
  subject,
  onSubjectChange,
  ...
  const { data: preview, isLoading: previewLoading } = useInvitePreview(opportunityId, userType, isResend);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await previewRefresh.mutateAsync({ opportunityId, userType, subject, body, isResend });
        ...
        {previewLoading ? (
          ...
          <VStack align="stretch" gap={3}>
            <VStack align="stretch" gap={1}>
              ...
              <Input
                value={subject}
                onChange={(e) => { onSubjectChange(e.target.value); validateField(e.target.value, setSubjectError); }}
            ...
            <VStack align="stretch" gap={1}>
              ...
              <Textarea
                value={body}
                onChange={(e) => { onBodyChange(e.target.value); validateField(e.target.value, setBodyError); }}
