...
function useLiveAgent(isActive: boolean) {
  ...
  useEffect(() => {
    ...
    if (document.getElementById("la_x2s6df8d_init")) return;
    ...
    // No cleanup — keep LiveAgent alive so re-opens work correctly
  }, [
    isActive,
    user,
    getUserFirstName,
  ...
  const openChat = () => window._laChatButton?.onClick();

  return { openChat };
}

interface SupportContentProps {
  /** Pass false to defer LiveAgent init (e.g. while a modal is closed). Defaults to true. */
  isActive?: boolean;
}

export function SupportContent({ isActive = true }: SupportContentProps) {
  const { openChat } = useLiveAgent(isActive);
