...
export const GeocodeAutocompleteInput = memo(({
  ...
  const controller = control && name ? useController({
    name,
    control,
  }) : null;
  ...
  useEffect(() => {
    ...
    if (debouncedInputValue && debouncedInputValue.trim().length >= 2) {
      ...
      mutationRef.current(trimmedAddress)
        .then((response) => {
          if (response && response.formatted_address && response.latitude && response.longitude) {
            setResults([response]);
          } else {
            setResults([]);
          }
...
<Box position="relative" width="100%">
          ...
          <Input
            ...
            fontSize="16px"
            px={icon ? 12 : 6}
            {...(controller ? controller.field : { value: inputValue || "" })}
